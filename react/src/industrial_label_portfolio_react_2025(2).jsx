...
import { ArrowUpRight, Mail, Github, Linkedin, Download, Filter, Scissors } from "lucide-react";
...
function Barcode({ value = "PORTFOLIO-2025", height = 48, density = 3 }) {
  ...
  const bars = useMemo(() => {
    ...
    arr.unshift({ w: 2, black: true }, { w: 1, black: false }, { w: 2, black: true });
    arr.push({ w: 2, black: true }, { w: 1, black: false }, { w: 2, black: true });
    return arr;
  }, [value]);
  ...
  return (
    <svg viewBox={`0 0 ${width} ${height}`} width="100%" height={height} aria-label={`barcode ${value}`} role="img">
      {/* Draw the bars left→right by accumulating x */}
      {bars.map((b, i) => {
        const rect = (
          <rect key={i} x={x} y={0} width={b.w * density} height={height} fill={b.black ? "#111" : "#fff"} />
        );
        x += b.w * density;
        return rect;
      })}
      ...
      <text x={8} y={height - 3} fontFamily="ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, 'Liberation Mono', 'Courier New', monospace" fontSize="10" fill="#111">
        {value}
      </text>
    </svg>
  );
...
function RealQR({ data, size = 108 }) {
  ...
  return ok ? (
    <img src={src} width={size} height={size} alt="QR" onError={() => setOk(false)} className="block select-none" loading="lazy" />
  ) : (
...
function FauxQR({ seed = "PORTFOLIO-QR", size = 108, modules = 21 }) {
  ...
  const rng = mulberry32([...seed].reduce((a, c) => a + c.charCodeAt(0), 0) || 1);
  ...
  for (let y = 0; y < modules; y++) {
    for (let x = 0; x < modules; x++) {
      ...
      const inFinder = boxes.some((b) => x >= b.x && x < b.x + b.s && y >= b.y && y < b.y + b.s);
      let on = false;
      if (inFinder) {
  ...
  return (
    <svg viewBox={`0 0 ${size} ${size}`} width={size} height={size} role="img" aria-label="decorative QR code">
      <rect width={size} height={size} fill="#fff" />
      {cells.map((c, i) => (c.on ? <rect key={i} x={c.x * cell} y={c.y * cell} width={Math.ceil(cell)} height={Math.ceil(cell)} fill="#111" /> : null))}
    </svg>
  );
}
...
function ProjectCard({ p }) {
  ...
  return (
    <motion.article
      ...
      className="relative group border border-black bg-white text-black shadow-[0_0_0_1px_#000] overflow-hidden" style={thermalPaperBg}
    >
      ...
      <div className="p-4 grid gap-3">
        ...
        {items.length ? (
          <div>
            <div className="px-2 py-1 text-[10px] uppercase font-semibold border-b border-black">Products</div>
            <div className="p-2">
              {items.map((it, idx) => (
                <div key={idx} className="grid grid-cols-[auto_1fr_auto] items-baseline gap-3 py-1 border-b border-dashed border-black text-[12px]">
          ...
          <div className="px-2 py-1 text-[10px] uppercase border border-dashed border-black">No products</div>
        )}
        ...
        <div className="mt-1">
          <div className="grid grid-cols-[1fr_auto] gap-3 py-1 text-[11px] uppercase">
            ...
            <span className="font-mono">{p.hours != null ? `${p.hours}h` : "—"}</span>
          </div>
          <div className="grid grid-cols-[1fr_auto] gap-3 py-1 border-t border-dashed border-black text-[11px] uppercase">
            <span className="tracking-wide font-semibold">Read Time (Case)</span>
        ...
        <div className="flex items-center justify-between pt-2">
          <div className="w-1/2 max-w-[220px]"><Barcode value={`${p.id}-${p.year}`} height={36} /></div>
          <a
            href={p.link}
            target="_blank"
            rel="noreferrer"
        ...
        <div className="grid grid-cols-[1fr_auto] gap-3 items-center py-2">
          <div className="text-[10px] uppercase">
            <div className="flex items-center gap-2"><Scissors size={14} /><span>Cut along the perforation · Keep stub</span></div>
            <div className="font-mono mt-1">{p.id}/{p.year} · LOT {p.lot}</div>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-[96px]"><Barcode value={`STUB-${p.id}`} height={32} /></div>
            <RealQR data={p.link} size={80} />
          </div>
        </div>
        <p className="text-[10px] uppercase tracking-wide">No returns · Keep for your records · Printed {new Date().toISOString().slice(0, 10)}</p>
      </div>
    </motion.article>
  );
}
...
const DEMO = [
  {
    ...
    readMinutes: 2,
    image: null,
    summary: "Built a reusable, tokenized component library adopted by 3+ internal teams; set standards (tokens, naming, best practices), maintained docs/versioning, and led 2 onboarding workshops to drive adoption. ",
  ...
  },
  {
    ...
    readMinutes: 3,
    image: null,
    summary: "Developed a scalable design system used across multiple product teams; ran interviews, surveys, and usability tests to validate needs; iterated user flows to enhance usability and satisfaction. ",
    link: "https://nestux.site",
  },
  {
    id: "x03",
    ...
    readMinutes: 3,
    image: null,
    summary: "Produced wireframes/prototypes, aligned stakeholders, and shipped responsive, cross-browser websites; identified improvements, balanced business/user needs, and supported marketing/process definition (Figma, WordPress, YOOtheme). ",
    link: "https://nestux.site",
  },
];
...
export default function IndustrialPortfolio() {
  ...
  const projects = useMemo(() => {
    return DEMO.filter((p) =>
      (q ? (p.title + p.summary + p.tags.join(" ")).toLowerCase().includes(q.toLowerCase()) : true) &&
  ...
  return (
    <div className="min-h-screen bg-white text-black">
      ...
      <div className="pointer-events-none fixed inset-0 [background:repeating-linear-gradient(0deg,#e5e5e5_0px,#e5e5e5_1px,transparent_1px,transparent_24px),repeating-linear-gradient(90deg,#f0f0f0_0px,#f0f0f0_1px,transparent_1px,transparent_24px)]" aria-hidden />
      ...
      <header className="sticky top-0 z-40 border-b border-black bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/70">
        <div className="mx-auto max-w-6xl px-4 py-3 flex items-center gap-4">
          ...
          <div className="flex-1 leading-tight">
            <h1 className="font-black uppercase tracking-[-0.02em] text-[clamp(18px,2.6vw,28px)]">Utilitarian Portfolio: Technical Aesthetic</h1>
            <p className="uppercase text-[10px]">spec: bw/hi-contrast · grid: 24px · medium: web · origin: earth · qc: passed</p>
          </div>
          <nav className="hidden md:flex items-center gap-3 text-[11px] uppercase">
            <a href="#work" className="px-2 py-1 border border-black hover:bg-black hover:text-white">Work</a>
            <a href="#about" className="px-2 py-1 border border-black hover:bg-black hover:text-white">About</a>
            <a href="#contact" className="px-2 py-1 border border-black hover:bg-black hover:text-white">Contact</a>
          </nav>
        </div>
      </header>
      ...
      <section className="mx-auto max-w-6xl px-4 py-10 grid md:grid-cols-[1fr_auto] gap-8 items-start">
        <div className="grid gap-4">
          <div className="border border-black p-4">
            <h2 className="font-black uppercase text-[clamp(24px,6vw,72px)] leading-[0.95] tracking-tight">Brutalist, Minimal, Systematic.</h2>
            <p className="mt-3 max-w-[60ch] text-[15px] md:text-base font-medium">I build typography‑driven interfaces and industrial brand systems. This template leans into label codes, strict grids, and honest black/white contrast for a durable portfolio presence.</p>
            <div className="mt-4 flex flex-wrap items-center gap-2 text-[10px] uppercase">
              <span className="px-2 py-1 border border-black">performance ≤ 1s lcp</span>
              <span className="px-2 py-1 border border-black">a11y AA</span>
              <span className="px-2 py-1 border border-black">responsive grid</span>
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            ...
            <div className="border border-black p-3">
              ...
              <p className="font-mono text-xl">{new Date().toISOString().slice(0, 10)}</p>
            </div>
          </div>
        </div>
      ...
      <section id="work" className="mx-auto max-w-6xl px-4 pb-3">
        <div className="flex flex-wrap items-center gap-3 border border-black p-3">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {selected.length > 0 && (
              <button onClick={() => setSelected([])} className="px-2 py-1 text-[11px] uppercase border border-dashed border-black">
                Clear
              </button>
            )}
          </div>

          <span className="ml-auto text-[11px] uppercase">{projects.length} / {DEMO.length} shown</span>
        </div>
      </section>
      ...
      {/* About / Credentials */}
      <section id="about" className="mx-auto max-w-6xl px-4 pb-16 grid lg:grid-cols-3 gap-8 items-start">
        <div className="border border-black p-5 bg-white">
          <h3 className="font-black uppercase text-2xl">About</h3>
          <p className="mt-3 font-medium max-w-prose">Designer/engineer focused on utilitarian visual systems. I like grids, label vernacular, and readable code. I work end‑to‑end: research → IA → UI → engineering → performance QA.</p>
          <ul className="mt-4 grid sm:grid-cols-2 gap-2 text-[12px]">
            <li className="border border-black p-2">Grid discipline: <b>24px</b> baseline</li>
            <li className="border border-black p-2">Type stack: <b>Inter / Mono</b></li>
            <li className="border border-black p-2">Perf budget: <b>170kb</b> JS</li>
            <li className="border border-black p-2">A11y: <b>Keyboard + SR</b></li>
          </ul>
        </div>
        <div className="border border-black p-5 bg-white">
          ...
          <div className="mt-3 grid gap-2 text-[12px]">
            {[
              ...
              <div key={i} className="flex items-center justify-between border border-black p-3">
                <span className="uppercase">{s}</span>
                <span className="font-mono">SVC-{(i + 1).toString().padStart(3, "0")}</span>
              </div>
            ))}
          </div>
        </div>
        <div className="border border-black p-5 bg-white">
          ...
          <ol className="mt-3 space-y-2 text-[12px]">
            <li className="border border-black p-3 flex items-center justify-between"><span>HSE · Visual Research in Utilitarian Design</span><span className="font-mono">DOC‑2018</span></li>
            <li className="border border-black p-3 flex items-center justify-between"><span>Data‑Aesthetic Thesis · Bartlett</span><span className="font-mono">TH‑2017</span></li>
            <li className="border border-black p-3 flex items-center justify-between"><span>WCAG 2.2 Practitioner</span><span className="font-mono">CERT‑AA</span></li>
          </ol>
          <a href="#" className="mt-4 inline-flex items-center gap-2 px-3 py-2 border border-black uppercase text-[11px] font-bold hover:bg-black hover:text-white"><Download size={16}/> Download Resume</a>
        </div>
      </section>
      ...
      <footer id="contact" className="border-t border-black bg-white/80">
        <div className="mx-auto max-w-6xl px-4 py-10 grid md:grid-cols-[1fr_auto] gap-6 items-start">
          <div>
            ...
            <p className="mt-2 text-sm max-w-prose">Available for select collaborations. Quote number on correspondence for faster routing.</p>
            <div className="mt-4 grid sm:grid-cols-3 gap-3 text-[12px]">
              <div className="border border-black p-3">
                <p className="uppercase">Primary</p>
                <a className="font-mono flex items-center gap-2 mt-1 hover:underline" href="mailto:contact@yourname"> <Mail size={16}/> contact@yourname</a>
              </div>
              <div className="border border-black p-3">
                <p className="uppercase">Networks</p>
                <div className="flex items-center gap-3 mt-1">
                  <a className="font-mono hover:underline" href="#"><Github size={16} className="inline"/> GitHub</a>
                  <a className="font-mono hover:underline" href="#"><Linkedin size={16} className="inline"/> LinkedIn</a>
                </div>
              </div>
              <div className="border border-black p-3">
          ...
          <div className="w-32 self-center md:self-start">
            ...
            <p className="text-[10px] uppercase text-center mt-1">made in web · ce</p>
          </div>
        </div>
        <div className="border-t border-black py-4 text-center text-[10px] uppercase">© {new Date().getFullYear()} Your Name — utilitarian portfolio · all rights reserved</div>
      </footer>

      {/* Accessibility: skip link */}
      <a href="#work" className="sr-only focus:not-sr-only fixed top-2 left-2 bg-white border border-black px-3 py-2 text-[12px]">Skip to work</a>
